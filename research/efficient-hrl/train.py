...
def collect_experience(tf_env, agent, meta_agent, state_preprocess,
  ...
  Args:
    tf_env: A TFEnvironment.  (/home/noobuntu/VEnvs/pytorch_env/lib/python3.7/site-packages/tf_agents/environments/tf_environment.py)
    agent: A UVF agent.
  ...
  with tf.control_dependencies([state]):
    transition_type, reward, discount = tf_env.step(action)
      # WTF is transition_type : next_step.is_last()[0] -> bool whether or not episode finishes? But seems to only check for n=100. If at all.
  ...
  def update_episode_rewards(context_reward, meta_reward, reset):
    ...
    # Seems to :
    #   Prepend 0 to the reward sequence if not 'reset'
    #   Otherwise, add the context reward / meta reward, respectively, to the first position in the reward sequences
    return tf.group(
  ...
  def no_op_int():
   ...
   # step_cond_fn: A function indicating whether or not to increase the step....
  ...
  with tf.control_dependencies([increment_op, reward, discount]):
    ...
    # transition_type = tf.Print(transition_type, ["\ntransition type: (train.py/collect experience):\n",
    #                                              transition_type])
...
#
# def collect_experience_OLD_PREVIOUS(tf_env, agent, meta_agent, state_preprocess,
#                        replay_buffer, meta_replay_buffer,
#                        action_fn, meta_action_fn,
#                        environment_steps, num_episodes, num_resets,
#                        episode_rewards, episode_meta_rewards,
  ...
  for mode in ['meta', 'nometa']:
    ...
    with tf.name_scope(mode):
      ...
      with tf.name_scope('Reward'):
        ...
        #tf.print([mode, "10 of current 100 rewards:", rewards[:10]])
